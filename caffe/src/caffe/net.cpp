...
Dtype Net<Dtype>::ForwardFromTo(int start, int end) {
  ...
  Dtype loss = 0;
  // flag to record Relu sparcity
  int ReluRecordFlag = 1;
  int ReluFlag = 0;
...
void Net<Dtype>::ForwardReluRecord(const int layer_id, int& reluFlag) {
    for (int top_id = 0; top_id < top_vecs_[layer_id].size(); ++top_id) {
        if (layer_names_[layer_id][0] == 'c' && reluFlag == 1) {
            int all = int(bottom_vecs_[layer_id][top_id]->count());
            int zero = 0;
            for (int i = 0; i<int(bottom_vecs_[layer_id][top_id]->count()); i++)
                if (float(bottom_vecs_[layer_id][0]->cpu_data()[i]) == float(0))
                    zero++;

            LOG_IF(INFO, Caffe::root_solver())
                << " Layer name is:  " << layer_names_[layer_id]
                << " Compression ratio is:  " << float(zero) / float(all) * 100;
            reluFlag = 0;
        }
        if (layer_names_[layer_id][0] != 'r' && layer_names_[layer_id][0] != 'p') {
            reluFlag = 0;
        }
    }
}
